...
- [The Implementation](#the-implementation)
  ...
  - [The code](#the-code)
    ...
    - [II. Algorithm Description](#ii-algorithm-description)
      ...
      - [5. Sprite Encodage and Display](#5-sprite-encodage-and-display)
...
Future organigram to describe the evolution of the state machine, interaction between the different modules and memory units.
...
In this part, we will describe how the different modules are connected to each other.
...
Thus the memory will be as many blocks of bits as there are sprites to display. Each block will contain the information of a sprite : its position, its color, and its identifier.
It will be single port memory, that means that only one block can be read or written at a time, as we won't need to write in it during the game.
...
You will find in these documents all the information needed to implement a memory block in Verilog, ad that is how the memory will look like :

![Memory block](data/ram.png)

In a RAM, the width is the number of bits of each block, and the depth is the number of blocks.   
In our case, the width will be 9 bits (3 bits for each Red, Green and Blue components), and the depth will be 1024 blocks (the number of pixels in a sprite).
...
The VGA display is handled by 3 modules and we won't need to fully explain them as it is related to VGA working process. This VGA chip works with a 60Hz refresh rate, and that the VGA screen is composed of 640x480 pixels.
...
Exemple to write a green pixel on the middle of the screen :
```verilog
...
However, each color will be assign in a 9 bits wide register and will be written as :
```verilog
...
#### 5. Sprite Encodage and Display

Each sprite wil be stored in the memory using an adaptative encodage to ptimize space.
...
Then, when we need to access the memory to display the color, we simply read the color index table of this sprite and make each 3 bits index match with the corresponding 9 bits color and display it.
